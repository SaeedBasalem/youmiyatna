...
const SMALL_THINGS = ["كوب القهوة الصباحي", "رسالة قبل النوم", "ضحكة بلا سبب", "مشوار بعد المغرب", "أكلة تطبخها لنا", "أغنية تشغّلها فجأة", "صمتٍ مريح بيننا", "دعوة بظهر الغيب", "لمسة يد عابرة", "تصبيرة وقت التعب"];
...
const CONVO_TEMPLATES = [
  (a) => `متى آخر مرة شعرتَ فيها ب${a.f} بسببي؟`,
...
/* ==================== لو خيّروك ==================== */
const WYR_A = ["نعيش سنة في " , "نقضي شهرًا في ", "نصحو كل صباح في ", "نقضي إجازتنا في "];
...
/* ==================== عجلة السهرة ==================== */
// Each activity carries where it happens, and each twist where it fits. These
// used to be a verb list and an object list multiplied together, which made
// most of the wheel nonsense — "نقرأ فيلم قديم", "نطبخ شارع لم نمشِ فيه",
// "نمشي في قائمة أمنياتنا". Now only combinations that make sense are built.
const DATE_ACTS = [
  ["نشاهد فيلمًا قديمًا نحبّه", "sit"], ["نقرأ صفحات من كتاب واحد", "sit"],
  ["نقلّب ألبوم صورنا", "sit"], ["نخطّط لسفرتنا القادمة", "sit"],
  ["نكتب قائمة أمنياتنا", "sit"], ["نكتب لبعضنا رسالة", "sit"],
  ["نحكي قصصًا من طفولتنا", "sit"], ["نلعب لعبةً كنّا نحبّها صغارًا", "sit"],
  ["نتعلّم شيئًا جديدًا سوا", "sit"],
  ["نطبخ أكلةً جديدة", "do"], ["نحضّر حلا بسيط", "do"], ["نخبز خبزًا في البيت", "do"],
  ["نعدّ عشاءً بسيطًا معًا", "do"], ["نغيّر ركنًا في بيتنا", "do"],
  ["نمشي في شارعٍ لم نمشِ فيه", "out"], ["نجرّب مطعمًا لم نزره", "out"],
  ["نروح مشوارًا بالسيارة بلا وجهة", "out"], ["نزور مكانًا من ذكرياتنا", "out"],
  ["نصلّي في مسجدٍ جديد ثم نتمشّى", "out"],
];
const DATE_TWISTS = [
  ["على ضوء الشموع 🕯️", "sit"], ["على السطح 🌙", "sit"], ["مع قهوة الليل ☕", "sit"],
  ["بلا جوّالات 📵", "any"], ["مع أغانينا 🎵", "any"], ["ونحن نضحك بلا سبب 😄", "any"],
  ["ونختمها بدعاء 🤲", "any"], ["بمفاجأة صغيرة 🎁", "any"],
];
const DATE_COMBOS = [];
for (const [a, where] of DATE_ACTS) for (const [t, fits] of DATE_TWISTS) if (fits === "any" || fits === where) DATE_COMBOS.push(a + " " + t);
export function dateIdea(opts) {
  return draw("date", (i) => DATE_COMBOS[i % DATE_COMBOS.length], DATE_COMBOS.length, opts);
}
...
/* ==================== صراحة ==================== */
// One of them answers about themself and the other guesses — the same shape as
// كم تعرفني, but the questions go a step closer.
const TRUTH_SUBJ = [
  "أكثر لحظة خفت فيها أن تخسرني", "شيء سامحتني عليه ولم تقله", "أكثر ما تخاف أن يتغيّر فينا",
  "أمنية لك لم تخبرني بها", "أكثر ما تحتاجه مني هذه الأيام", "شيء تتمنى لو بدأناه أبكر",
  "أكثر عادة عندي تتحمّلها بصمت", "شيء فعلته وأسعدك ولم ألحظه", "أكثر ما يشعرك بالأمان معي",
  "شيء تريد أن نتوقّف عنه", "أكثر يوم تمنيت لو يعود", "ما الذي يزعجك ولا تقوله",
  "أكثر ما تفخر به فيّ", "شيء تريد أن نتعلّمه قبل أن نكبر", "أكثر ما يرهقك ولا أراه",
  "شيء تحبه في بيتنا", "أكثر ما تنتظره في يومك", "دعوة تدعوها لي ولا تسمعني إياها",
];
const TRUTH_FRAME = ["بصراحة: ما ", "بلا مجاملة — ما ", "قل الصدق: ما "];
export function truthQ(opts) {
  const total = TRUTH_SUBJ.length * TRUTH_FRAME.length;
  return draw("truth", (i) => pick(TRUTH_FRAME, Math.floor(i / TRUTH_SUBJ.length)) + pick(TRUTH_SUBJ, i) + "؟", total, opts);
}

/* ==================== أكمل الجملة ==================== */
// A sentence with its end missing: one finishes it honestly, the other guesses
// how it ended.
const FINISH_STEMS = [
  "أكثر ما يريحني بعد يوم طويل هو…", "لو ملكنا يومًا كاملًا بلا التزامات لـ…",
  "أشعر أنك قريب مني حين…", "أكثر ما أتمناه لنا هذه السنة هو…",
  "أضحك دائمًا حين تتذكّر…", "أول شيء سأفعله حين نصل بيتنا الجديد هو…",
  "الشيء الذي لا أمل تكراره معك هو…", "حين أتعب، أحتاج منك أن…",
  "أجمل ما في صباحاتنا هو…", "أكثر مكان أشعر فيه أننا نحن هو…",
  "لو كتبت رسالة تُفتح بعد عشر سنين لبدأتها بـ…", "أكثر ما تعلّمته منك هو…",
  "أشعر بالامتنان حين…", "أصعب ما مررنا به علّمنا أن…",
  "الشيء الصغير الذي أفتقده حين تغيب هو…", "أحب أن أسمعك تقول…",
  "أكثر عادة لنا أتمنى ألا تنتهي هي…", "حين أفكر في بيتنا بعد سنين أرى…",
];
const FINISH_FRAME = ["أكمل: ", "كمّل الجملة — ", ""];
export function finishLine(opts) {
  const total = FINISH_STEMS.length * FINISH_FRAME.length;
  return draw("finish", (i) => pick(FINISH_FRAME, Math.floor(i / FINISH_STEMS.length)) + pick(FINISH_STEMS, i), total, opts);
}
...
const CH_BODY = [
  ...
  (s) => `ادعُوَا لبعضكما كل ليلة بدعوةٍ من القلب، وقولاها بصوت.`,
...
const SHOT_VERB = ["صوّرا", "التقطا", "سجّلا صوتكما وأنتما تصفان"];
...
const KHALWA_SOURCES = [
  ...
  { theme: "النيّة في الإنفاق", source: "«إنك لن تُنفق نفقةً تبتغي بها وجه الله إلا أُجرت عليها، حتى ما تجعل في فم امرأتك» — متفق عليه" },
